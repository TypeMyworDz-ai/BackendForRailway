...
RUN echo "--- Attempting to install deepgram-sdk in isolation ---" && \
    ...
    python -c "from deepgram.client import DeepgramClient; print('Deepgram SDK (DeepgramClient) imported successfully.')" || \
    (echo "!!! ERROR: Deepgram SDK failed to import DeepgramClient from deepgram.client after isolated install. Check above logs for details. !!!" && exit 1)
...
RUN echo "--- Verifying Deepgram SDK again after all requirements.txt installs ---" && \
    python -c "from deepgram.client import DeepgramClient; print('Deepgram SDK (DeepgramClient) imported successfully (post-all-install).')" || \
    (echo "!!! ERROR: Deepgram SDK failed to import DeepgramClient from deepgram.client after all other installs. A dependency conflict might exist. !!!" && exit 1)
